...
服务默认监听 3000 端口，可通过 `PORT` 环境变量覆盖。本地 `skill_dashboard` 使用 `PORT=3001` 启动，避免与其他本地服务冲突。
...
| 2026-09-11 | **支持可配置监听端口**：服务读取 `PORT` 环境变量并保留 3000 默认值，供本地技能仪表盘在 3001 端口启动；新增端口配置单元测试。 |
